...
static char *s_buff;    // 缓存,esp8266的栈比较小,所以使用堆区内存
...
int config_light_get_data(const char *json, p_config_light light)
{
    ...
    cJSON *name;
    cJSON *color;
    p_config_light_item light_item;

    count = cJSON_GetArraySize(root); /*获取数组长度*/

    if (count > LIGHT_MAX_COUNT)
    {
        count = LIGHT_MAX_COUNT;
    }

    light->max = LIGHT_MAX_COUNT;
    light->count = count;

    for (i = 0; i < count; i++)
    {
        item = cJSON_GetArrayItem(root, i);
        light_item = &(light->item[i]);
        ...
        strncpy(light_item->name, name->valuestring, sizeof(light_item->name) - 1);
        ...
        light_item->color = color->valueint;
...
int config_light_get_json(const p_config_light light, char *json)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        len += sprintf(&json[len],
                       "{\"%s\":%d,\"%s\":%d,\"%s\":\"%s\",\"%s\":%d},",
                       CONFIG_LIGHT_ID,    item->id,
                       CONFIG_LIGHT_ON,    item->on,
                       CONFIG_LIGHT_NAME,  item->name,
                       CONFIG_LIGHT_COLOR, item->color);
